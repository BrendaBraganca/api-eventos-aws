...
public interface EventRepository extends JpaRepository<Event, UUID>{
    ...
    @Query("SELECT e FROM Event e LEFT JOIN e.address a "+ "WHERE (:title = '' OR e.title LIKE %:title%)" + "AND (:city = '' OR a.city LIKE %:city%)" + "AND (e.date >= :startDate AND e.date <= :endDate)")
